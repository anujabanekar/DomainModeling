...
public class Gui extends java.awt.event.WindowAdapter implements java.awt.event.
   ...
   private JMenuItem developerMenuItem=null;
   ...
   //
   private JDialog developerDialog=null; //  @jve:decl-index=0:visual-constraint="35,704"
   private JPanel developerContentPane=null;
   private JLabel developerVersionLabel=null;
   //
...
//	}
   ...
   private JMenu getFileMenu() {
      if (fileMenu == null) {
         ...
         fileMenu.add(getOpenExistingWork());
         //fileMenu.add(getSaveAndContinueWork());
   ...
   private JMenu getHelpMenu() {
      if (helpMenu == null) {
         ...
         helpMenu.add(getDeveloperMenuItem());
      }
      return helpMenu;
   }
   ...
   private JMenuItem getDeveloperMenuItem() {
	      if (developerMenuItem == null) {
	    	  developerMenuItem=new JMenuItem();
	    	  developerMenuItem.setText("Developers");
	    	  developerMenuItem.setToolTipText(
	            "Click here to know the developers for this version");
	    	  developerMenuItem.addActionListener(new ActionListener() {
	            public void actionPerformed(ActionEvent e) {
	               JDialog aboutDialog=getDeveloperDialog();
	               aboutDialog.pack();
	               Point loc=getJFrame().getLocation();
	               loc.translate(20, 20);
	               aboutDialog.setLocation(loc);
   ...
   /** Developer details for this version of tool - start 
    */
   
   private JDialog getDeveloperDialog() {
	      if (developerDialog == null) {
	    	  developerDialog=new JDialog(getJFrame(), true);
	    	  developerDialog.setTitle("Developers");
	    	  developerDialog.setContentPane(getDeveloperContentPane());
	      }
	    ...
	    * @return javax.swing.JPanel
	    */
	   private JPanel getDeveloperContentPane() {
	      if (developerContentPane == null) {
	    	  developerContentPane=new JPanel();
	    	  developerContentPane.setLayout(new BorderLayout());
	         JPanel jp=new JPanel(new BorderLayout());
	         JPanel aboutPanel=new JPanel(new BorderLayout());
	         developerContentPane.add(jp, BorderLayout.CENTER);
	         jp.add(aboutPanel, BorderLayout.CENTER);
	         JLabel tool=new JLabel("  Ankit Kurchania, Anuja Banekar, Mahesh Hingane,  ");
	         tool.setHorizontalAlignment(tool.CENTER);
	         aboutPanel.add(tool, BorderLayout.NORTH);
	         JLabel vendor=new JLabel("  Qicheng Wang, Sneha Madhuri Patapatnam.  ");
	         vendor.setHorizontalAlignment(vendor.CENTER);
	         aboutPanel.add(vendor, BorderLayout.CENTER);
	         ...
	         closeButton.addActionListener(new ActionListener() {
	            public void actionPerformed(ActionEvent e) {
	               developerDialog.setVisible(false);
	            }
	         }
	         );
	      }
	      return developerContentPane;
	   }

	
	// Adding developer details - end
   ...
   private JMenuItem getOpenMenuItem() {
      if (openMenuItem == null) {
         ...
         openMenuItem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent e) {
            	//Check if text already exists, if yes then show pop up - start
            	if(getEditorTextPane().getText() != null)
            	{
            		int option=JOptionPane.showConfirmDialog(jFrame,
            	            "Do you want append the text?",
            	            "", JOptionPane.YES_NO_OPTION,
            	            JOptionPane.YES_OPTION);
            	         if (option == JOptionPane.YES_OPTION) {
            	          String prevText = getEditorTextPane().getText();
            	          ...
            	          if (returnVal == JFileChooser.APPROVE_OPTION) {
            	             ...
            	             try {
            	                ...
            	                reader.close();
            	                getEditorTextPane().setText(prevText + text);
   ...
   private JMenuItem getOpenExistingWork() {
